...
public final class TitleBox {
    ...
    public void render(DrawContext ctx) {
        ...
        if (selected == null) {
            ...
            int tw = font.getWidth(placeholder);
            int tx = Math.round(x + (w / 2f) - (tw / 2f));
            int ty = Math.round(y + (h / 2f) - (font.fontHeight / 2f));
            ctx.drawTextWithShadow(font, placeholder, tx, ty, 0xFFFFFFFF);
            updateButtonLayout();
            updateButtonVisibility();
            return;
        }
