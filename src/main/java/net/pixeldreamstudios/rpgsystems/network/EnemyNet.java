...
public final class EnemyNet {
    ...
    public static void broadcastDamageNumber(Entity target, float amount, boolean crit, boolean isPet, int rgb, UUID sourceUuid) {
        if (target == null || target.getServer() == null) return;
        var pkt = new EnemyHudPayloads.DamageNumber(target.getId(), amount, crit, isPet, rgb, sourceUuid);
        forNearbyPlayers(target, sp -> ServerPlayNetworking.send(sp, pkt));
    }

    public static void broadcastHealingNumber(Entity target, float amount, boolean isSpell, UUID sourceUuid) {
        if (target == null || target.getServer() == null) return;
        var pkt = new EnemyHudPayloads.HealingNumber(target.getId(), amount, isSpell, sourceUuid);
        forNearbyPlayers(target, sp -> ServerPlayNetworking.send(sp, pkt));
    }
