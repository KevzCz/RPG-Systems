        ...
        if (raw.startsWith("/p")) {
            ...
            switch (sub) {
                case "help" -> {
                    ...
                    """.trim();
                    sendNoticeTo(sender, p, help, now);
                }
                case "info" -> {
                    String leaderName = state.nameOf(p.leader);
                    if (leaderName == null) {
                    ...
                    int online = 0;
                    for (UUID u : p.members) if (sender.getServer().getPlayerManager().getPlayer(u) != null) online++;
                    String info = "Party: " + displayName(p) + " | Leader: " + leaderName + " | Members: " + p.members.size() + " (" + online + " online)";
                    sendNoticeTo(sender, p, info, now);
                }
                case "sharepos" -> {
                    ...
                    broadcastNotice(sender.getServer(), p, txt, now);
                }
                case "promote" -> {

                    if (!p.leader.equals(sender.getUuid())) {
                        sendNoticeTo(sender, p, "Only the party leader can promote.", now);
                        return;
                    }

                    if (parts.length < 3 || parts[2].isBlank()) {
                        sendNoticeTo(sender, p, "Usage: /p promote <member>", now);
                        return;
                    }
                    ...
                    if (targetUuid == null || targetUuid.equals(sender.getUuid())) {
                        sendNoticeTo(sender, p, "Couldn't resolve a unique member for \"" + parts[2] + "\".", now);
                        return;
                    }
                ...
                case "pin" -> {
                    ...
                    if (!p.leader.equals(sender.getUuid())) {
                        sendNoticeTo(sender, p, "Only the party leader can pin.", now);
                        return;
                    }

                    if (parts.length < 3 || parts[2].isBlank()) {
                        sendNoticeTo(sender, p, "Usage: /p pin <text>", now);
                        return;
                    }

                    var pin = new PartyChatPayloads.ChatPinSet(p.id, parts[2], now);
                    for (UUID u : p.members) {
                        ServerPlayerEntity sp = sender.getServer().getPlayerManager().getPlayer(u);
                ...
                case "unpin" -> {
                    if (!p.leader.equals(sender.getUuid())) {
                        sendNoticeTo(sender, p, "Only the party leader can unpin.", now);
                        return;
                    }
                    var clear = new PartyChatPayloads.ChatPinClear(p.id);
                    for (UUID u : p.members) {
                        ServerPlayerEntity sp = sender.getServer().getPlayerManager().getPlayer(u);
                        if (sp != null) ServerPlayNetworking.send(sp, clear);
                    }
                    sendNoticeTo(sender, p, "Pinned banner cleared.", now);
                }
                default -> sendNoticeTo(sender, p, "Unknown subcommand. Try /p help", now);
            }
            return;
        }
